...
# ev_battery_charge_start_stop=False
ev_battery_charge_start_stop = False
...
def main_prg():
    global sim_hour
    global sim_min
    global ev_battery_charge_start_stop
    ...
    while True:
        ...
        for i in range(0, seconds_per_hour):
            if ev_battery_charge_start_stop:
...
def station_info():
    ...
    if request.method == 'GET':
        charging_station_info = {"sim_time_hour": sim_hour,
                                 ...
                                 "ev_battery_charge_start_stop": ev_battery_charge_start_stop
                                 }
        return jsonify(charging_station_info)
    else:
        return jsonify({'error': 'Unsupported HTTP method'})
...
def base_load_info():
    if request.method == 'GET':
        return jsonify(base_load_residential_kwh)
    else:
        return jsonify({'error': 'Unsupported HTTP method'})
...
def charge_battery():
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                start_charge = json_input.get('charging', 0)
                # start_charge = json_input["charging"]
            except json.JSONDecodeError:
                return jsonify({'error': 'Invalid JSON input'})
            with global_lock:
                if start_charge == "on":
                    ev_battery_charge_start_stop = True
...
def discharge_battery():
    ...
    if request.method == 'POST':
        try:
            ...
            # return jsonify(result)
            try:
                discharge = json_input.get('discharging', 0)
